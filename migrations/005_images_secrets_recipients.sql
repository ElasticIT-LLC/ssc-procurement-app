...
-- MUST stay on a single line: the shell exec_sql runner splits statements on ';' followed by a
-- newline, so a multi-line DO block gets truncated mid-block (unterminated dollar-quoted string).
DO $do$ BEGIN BEGIN DROP POLICY IF EXISTS product_images_service_write ON storage.objects; CREATE POLICY product_images_service_write ON storage.objects FOR ALL TO service_role USING (bucket_id = 'product-images') WITH CHECK (bucket_id = 'product-images'); DROP POLICY IF EXISTS product_images_auth_read ON storage.objects; CREATE POLICY product_images_auth_read ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'product-images'); EXCEPTION WHEN insufficient_privilege THEN IF EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'storage' AND tablename = 'objects' AND policyname = 'product_images_service_write') AND EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'storage' AND tablename = 'objects' AND policyname = 'product_images_auth_read') THEN RAISE NOTICE 'product-images storage policies already exist; skipping recreation'; ELSE RAISE EXCEPTION 'product-images storage policies missing and role "%" does not own storage.objects; create product_images_service_write + product_images_auth_read as supabase_admin in the dashboard SQL editor, then re-upload', current_user; END IF; END; END $do$;
